...
if [ $stage -le 0 ]; then
  ...
  # use the rest of the training data as the "real" dev set for perplexities.
  # JS: Changed from original file.
  tail -n +$[$num_dev_sentences+1] < data/train/text | cut -d " " -f 2- >  ${dir}/data/real_dev_set.txt
